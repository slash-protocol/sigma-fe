...
function Stake() {
  ...
  const timeBalance = useSelector<IReduxState, string>(state => {
    return state.account.balances && state.account.balances.sigma;
  });
  ...
  const memoBalance = useSelector<IReduxState, string>(state => {
    return state.account.balances && state.account.balances.ssigma;
  });
  const stakeAllowance = useSelector<IReduxState, number>(state => {
    return state.account.staking && state.account.staking.sigma;
  });
  const unstakeAllowance = useSelector<IReduxState, number>(state => {
    return state.account.staking && state.account.staking.ssigma;
  });
  ...
  const hasAllowance = useCallback(
    (token: string) => {
      if (token === "sigma") return stakeAllowance > 0;
      if (token === "ssigma") return unstakeAllowance > 0;
      return 0;
    },
    [stakeAllowance],
  );
          ...
          <Grid className="stake-card-grid" container direction="column" spacing={2}>
            <Grid item>
              <div className="stake-card-header">
                <p className="stake-card-header-title">SIGMA Staking (Σ, Σ)</p>
                <RebaseTimer />
              </div>
            </Grid>
                ...
                <Grid container spacing={2}>
                      ...
                      <>
                        {stakingAPY ? (
                          <div>
                            ...
                            <Popper className="sigma-menu-popper tooltip" open={open} anchorEl={anchorEl} transition>
                              {({ TransitionProps }) => (
                  ...
                  <Grid item xs={6} sm={4} md={4} lg={4}>
                    <div className="stake-card-index">
                      ...
                      <p className="stake-card-metrics-value">
                        {currentIndex ? <>{trim(Number(currentIndex), 2)} SIGMA</> : <Skeleton width="150px" />}
                      </p>
                    </div>
                  </Grid>
            ...
            <div className="stake-card-area">
              {!address && (
                <div className="stake-card-wallet-notification">
                  ...
                  <p className="stake-card-wallet-desc-text">Connect your wallet to stake SIGMA tokens!</p>
                </div>
              )}
              {address && (
                <div>
                  <div className="stake-card-action-area">
                      ...
                      {view === 0 && (
                        <div className="stake-card-tab-panel">
                          {address && hasAllowance("sigma") ? (
                            ...
                            >
                              <p>{txnButtonText(pendingTransactions, "staking", "Stake SIGMA")}</p>
                            </div>
                          ) : (
                            <div
                              className="stake-card-tab-panel-btn"
                      ...
                      {view === 1 && (
                        <div className="stake-card-tab-panel">
                          {address && hasAllowance("ssigma") ? (
                            ...
                            >
                              <p>{txnButtonText(pendingTransactions, "unstaking", "Unstake SIGMA")}</p>
                            </div>
                          ) : (
                            <div
                              className="stake-card-tab-panel-btn"
                    ...
                    <div className="stake-card-action-help-text">
                      {address &&
                        ((!hasAllowance("sigma") && view === 0) || (!hasAllowance("ssigma") && view === 1)) && (
                          <p>
                            Note: The "Approve" transaction is only needed when staking/unstaking for the first sigma;
                  ...
                  <div className="stake-user-data">
                    <div className="data-row">
                      ...
                      <p className="data-row-value">
                        {isAppLoading ? <Skeleton width="80px" /> : <>{trim(Number(timeBalance), 4)} SIGMA</>}
                      </p>
                    </div>
                    ...
                    {Number(warmupBalance) > 0 && (
                      <>
                        ...
                        <div className="data-row">
                          ...
                          <p className="data-row-value">
                            {isAppLoading ? <Skeleton width="80px" /> : <>{trim(Number(warmupBalance), 4)} SIGMA</>}
                          </p>
                        </div>
                        ...
                        {Number(warmupBalance) < Number(gonsBalance) && (
                          <>
                            <div className="data-row">
                              ...
                              <p className="data-row-value">
                                {isAppLoading ? <Skeleton width="80px" /> : <>{trim(Number(gonsBalance), 4)} SIGMA</>}
                              </p>
                            </div>
                          </>
                        )}
                          ...
                          <p className="data-row-value">
                            {isAppLoading ? (
                              ...
                              <>
                                ...
                                >
                                  <p>{txnButtonText(pendingTransactions, "claim", "Claim sSIGMA")}</p>
                                </div>
                                <br />
                              </>
                            ) : (
                    ...
                    <div className="data-row">
                      ...
                      <p className="data-row-value">
                        {isAppLoading ? (
                          ...
                          <>
                            {trimmedMemoBalance} sSIGMA (${trimmedMemoBalanceInUSD})
                          </>
                        )}
                      </p>
                    </div>
                    ...
                    <div className="data-row">
                      ...
                      <p className="data-row-value">
                        {isAppLoading ? (
                          ...
                          <>
                            {nextRewardValue} sSIGMA (${trim(nextRewardInUSD, 2)})
                          </>
                        )}
                      </p>
                    </div>
