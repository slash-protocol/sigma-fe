...
import { SigmaTokenContract, sSigmaTokenContract, MimTokenContract, StakingContract } from "../../abi";
...
export const getBalances = createAsyncThunk(
  ...
  async ({ address, networkID, provider }: IGetBalances): Promise<IAccountBalances> => {
    ...
    const memoContract = new ethers.Contract(addresses.sSIGMA_ADDRESS, sSigmaTokenContract, provider);
    const memoBalance = await memoContract.balanceOf(address);
    const timeContract = new ethers.Contract(addresses.SIGMA_ADDRESS, SigmaTokenContract, provider);
    const timeBalance = await timeContract.balanceOf(address);

    return {
      balances: {
        ssigma: ethers.utils.formatUnits(memoBalance, "gwei"),
        sigma: ethers.utils.formatUnits(timeBalance, "gwei"),
      },
    };
  },
);
...
export const loadAccountDetails = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: ILoadAccountDetails): Promise<IUserAccountDetails> => {
    ...
    return {
      ...
      staking: {
        sigma: Number(stakeAllowance),
        ssigma: Number(unstakeAllowance),
      },
    };
  },
);
...
const initialState: IAccountSlice = {
  loading: true,
  bonds: {},
  balances: { ssigma: "", sigma: "" },
  staking: { sigma: 0, ssigma: 0 },
