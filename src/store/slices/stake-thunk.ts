...
import { StakingHelperContract, SigmaTokenContract, sSigmaTokenContract, StakingContract } from "../../abi";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const timeContract = new ethers.Contract(addresses.SIGMA_ADDRESS, SigmaTokenContract, signer);
    const memoContract = new ethers.Contract(addresses.sSIGMA_ADDRESS, sSigmaTokenContract, signer);
    ...
    try {
      ...
      const text = "Approve " + (token === "sigma" ? "Staking" : "Unstaking");
      const pendingTxnType = token === "sigma" ? "approve_staking" : "approve_unstaking";
