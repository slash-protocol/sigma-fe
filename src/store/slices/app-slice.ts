...
import { StakingContract, sSigmaTokenContract, SigmaTokenContract } from "../../abi";
...
export const loadAppDetails = createAsyncThunk(
  ...
  async ({ networkID, provider }: ILoadAppDetails) => {
    ...
    const sigmaContract = new ethers.Contract(addresses.SIGMA_ADDRESS, SigmaTokenContract, provider);

    const marketPrice = ((await getMarketPrice(networkID, provider)) / Math.pow(10, 9)) * mimPrice;

    const burnedSupply = (await sigmaContract.balanceOf(addresses.BURN_ADDRESS)) / Math.pow(10, 9);
    const totalSupply = (await sigmaContract.totalSupply()) / Math.pow(10, 9) - burnedSupply;
    const circSupply = (await ssigmaContract.circulatingSupply()) / Math.pow(10, 9);
    ...
    const timeBondsAmountsPromises = allBonds.map(bond => bond.getSigmaAmount(networkID, provider));
    ...
    const circ = await ssigmaContract.circulatingSupply();
